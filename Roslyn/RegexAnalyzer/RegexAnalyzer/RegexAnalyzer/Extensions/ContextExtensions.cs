...
namespace RegexAnalyzer.Extensions
{
    /// <summary>
    ...
    public static class ContextExtensions
    {
        ...
        public static InvocationExpressionSyntax GetMethod(this SyntaxNodeAnalysisContext context,
            string typeFullName, string methodName)
        {
            var invocationExpr = (InvocationExpressionSyntax)context.Node;
            var memberAccessExpr = invocationExpr.Expression as MemberAccessExpressionSyntax;
            if (memberAccessExpr?.Name.ToString() != methodName)
                return null;

            var memberSymbol =
                context.SemanticModel.GetSymbolInfo(memberAccessExpr)
                    .Symbol as IMethodSymbol;
            if (!memberSymbol?.ToString().StartsWith($"{typeFullName}.{methodName}") ?? true)
                return null;
